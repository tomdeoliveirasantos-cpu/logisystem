...
export default function CadastrosMotorista() {
  ...
  // Cancelar (excluir) convite — funciona para pendente, expirado E preenchido.
  // Para 'preenchido', precisa primeiro excluir o cadastro vinculado, porque o backend
  // bloqueia DELETE direto no convite preenchido (preserva integridade referencial).
  const cancelarConvite = async (c) => {
    const nome = c.nome_motorista || 'sem nome';
    const isPreenchido = c.status === 'preenchido';

    // Confirmação clara, diferente para cada caso
    const msg = isPreenchido
      ? `Excluir o convite e o cadastro de "${nome}"?\n\nEste convite já foi preenchido. Ao continuar, o cadastro vinculado será apagado junto (com os arquivos anexados). Esta ação não pode ser desfeita.`
      : `Cancelar o convite para "${nome}"?\n\nEle deixará de ser válido — quem tiver o link não conseguirá mais preencher.`;
    if (!confirm(msg)) return;

    try {
      // Caso preenchido: achar o cadastro vinculado e excluí-lo primeiro
      if (isPreenchido) {
        // Buscar lista de cadastros, achar o que veio deste convite
        const cads = await api.get('/motorista-cadastros');
        const cadVinculado = (cads || []).find(x => x.convite_id === c.id);
        if (cadVinculado) {
          if (cadVinculado.status === 'aprovado') {
            return showToast(
              'Este cadastro já foi aprovado — não pode ser excluído por aqui. Reprove-o primeiro pelo detalhe.',
              'error'
            );
          }
          await api.delete(`/motorista-cadastros/${cadVinculado.id}`);
          // O backend devolve o convite para "pendente" automaticamente, então agora o DELETE do convite funciona
        }
      }

      await api.delete(`/motorista-cadastros/convites/${c.id}`);
      showToast(isPreenchido
        ? `Convite e cadastro de "${nome}" excluídos.`
        : `Convite de "${nome}" cancelado.`);
      refetchConvites();
      refetch();
    } catch (e) {
      showToast(e.message, 'error');
                    ...
                    <tr key={c.id}>
                      ...
                      <td>
                        <div style={{display:'flex',gap:4,alignItems:'center',flexWrap:'wrap'}}>
                          ...
                          {/* Lixeira sempre visível — para pendente, preenchido E expirado.
                              A função cancelarConvite trata cada caso (preenchido = exclui cadastro vinculado também). */}
                          <button
                            className="btn btn-ghost btn-sm"
                            onClick={()=>cancelarConvite(c)}
                            title={c.status === 'preenchido' ? 'Excluir convite e cadastro vinculado' : 'Cancelar convite'}
                            style={{color:'var(--red)', fontSize:14, padding:'4px 8px'}}
                          >
                            🗑
                          </button>
                        </div>
