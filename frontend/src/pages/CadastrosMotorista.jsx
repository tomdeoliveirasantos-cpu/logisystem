...
export default function CadastrosMotorista() {
  ...
  // Cancelar (excluir) convite pendente
  const cancelarConvite = async (c) => {
    const nome = c.nome_motorista || 'sem nome';
    if (!confirm(`Cancelar o convite para "${nome}"?\n\nEle deixará de ser válido — quem tiver o link não conseguirá mais preencher.`)) return;
    try {
      await api.delete(`/motorista-cadastros/convites/${c.id}`);
      showToast(`Convite de "${nome}" cancelado.`);
      refetchConvites();
    } catch (e) {
      showToast(e.message, 'error');
    }
                        ...
                        {ativo && (
                          <div style={{display:'flex',gap:4,alignItems:'center'}}>
                            ...
                            <button
                              ...
                              title="Cancelar convite (excluir)"
