...
router.get('/', async (req, res, next) => {
  try {
    ...
    const params = [];
    if (tipo) { sql += ` WHERE tipo = $1`; params.push(tipo); }
  ...
  try {
    ...
    const { rows } = await db.query(
      `SELECT percentual FROM logi_reajustes_frete
       WHERE tipo = $1 AND data_vigencia <= $2`,
      [tipo, data]
    );
    let fator = 1.0;
  ...
  try {
    ...
    const { rows } = await db.query(
      `INSERT INTO logi_reajustes_frete (tipo, percentual, data_vigencia, descricao)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [tipo, percentual, data_vigencia, descricao || null]
    );
    res.json(rows[0]);
  } catch (err) { next(err); }
});
  ...
  try {
    ...
    const { rows } = await db.query(
      `UPDATE logi_reajustes_frete
       ...
       WHERE id = $5
       RETURNING *`,
      [tipo, percentual, data_vigencia, descricao, id]
    );
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    const { rowCount } = await db.query(
      `DELETE FROM logi_reajustes_frete WHERE id = $1`, [id]
    );
...
// ════════════════════════════════════════════════════════════════
module.exports.getFatorReajuste = async function (client, tipo, data) {
