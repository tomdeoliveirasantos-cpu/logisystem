      ...
      <div className="page-body">
        ...
        {tab === 'faturamento' && (
          <div>
            <div className="metrics-grid cols-4 mb-16 fade-up">
              <div className="metric-card"><div className="metric-label">Total rotas</div><div className="metric-value">{fatRows.reduce((s, r) => s + Number(r.total_rotas), 0)}</div></div>
              <div className="metric-card"><div className="metric-label">Total entregas</div><div className="metric-value">{fatRows.reduce((s, r) => s + Number(r.total_entregas), 0)}</div></div>
              <div className="metric-card"><div className="metric-label">Faturamento</div><div className="metric-value" style={{ fontSize: 20 }}>{fmt(fatRows.reduce((s, r) => s + Number(r.valor_receber), 0))}</div></div>
              <div className="metric-card"><div className="metric-label">Margem bruta</div><div className="metric-value" style={{ fontSize: 20, color: 'var(--green)' }}>{fmt(fatRows.reduce((s, r) => s + Number(r.margem), 0))}</div></div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16 }}>
              <div className="card fade-up fade-up-1">
                <div className="section-title mb-12">Faturamento vs Margem</div>
              ...
              <div className="card fade-up fade-up-2">
                <div className="section-title mb-12">Detalhamento por veículo</div>
        ...
        {tab === 'km' && (
          <div>
            {/* KPIs */}
            <div className="metrics-grid cols-4 mb-16 fade-up">
              <div className="metric-card"><div className="metric-label">KM Total (mês)</div><div className="metric-value" style={{ fontSize: 18 }}>{totalKmMes.toLocaleString('pt-BR')} km</div></div>
              <div className="metric-card"><div className="metric-label">Viagens com KM</div><div className="metric-value">{totalViagensMes}</div></div>
              <div className="metric-card"><div className="metric-label">KM Médio/viagem</div><div className="metric-value" style={{ fontSize: 18 }}>{totalViagensMes > 0 ? (totalKmMes / totalViagensMes).toFixed(0) : 0} km</div></div>
              <div className="metric-card"><div className="metric-label">Regiões atendidas</div><div className="metric-value">{(kmRegiao || []).length}</div></div>
            </div>
            ...
            {/* KM por Região */}
            <div className="card fade-up fade-up-1 mb-16">
              <div className="section-title mb-12">KM por Região — Eficiência</div>
              <div className="table-wrap">
                <table>
                  <thead><tr><th>Região</th><th>Viagens</th><th>KM Total</th><th>KM Médio</th><th>Faturado</th><th>R$/km</th></tr></thead>
                  <tbody>
                    {!(kmRegiao || []).length && (
                      <tr><td colSpan={6} style={{ textAlign: 'center', color: 'var(--text3)', padding: '24px 0', fontSize: 13 }}>Nenhum dado de KM</td></tr>
                    )}
                    {(kmRegiao || []).map(r => (
                      <tr key={r.regiao}>
